class AgentBody {
  ...
  AgentBody(Vec3D pos, Vec3D vel, int id, int bType, float tipRad, boolean locked) {
    this.pos = pos;
    this.vel = vel;
    this.id=id;
    this.locked = locked;
    ...
    maxForce = 0.05;
    body = new Body(bType, tipRad); // creates a standard Body, type 0 (3-fork)
    aligned = false;
    connected = false;
  }


  AgentBody(Vec3D pos, Vec3D vel, int id, float tipRad, boolean locked) {
    this(pos, vel, id, 0, tipRad, locked);
  }
